...
class FireStoreEventDao {
  ...
  Future<List<List<Event>>> getEvents() async {
    final queryResults = await _eventsCollection
        .where('completed', isNull:true)
        .where('storeId', isEqualTo:1234)
        .get();
    return queryResults.docs.map((doc) => _documentDataToEventList(doc.data())).toList();
  }
  Future<String> addEvent(List<Event> event) async {
    final docRef = await _eventsCollection.add(_eventListToDocumentData(event));
    return docRef.id;
  }
